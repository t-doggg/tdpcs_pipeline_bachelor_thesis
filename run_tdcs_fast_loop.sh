...
# Nimm Arguments, also die Pfade auf aus der ./base File
INFQ="$2"
OUTDIR="$4"
REFHUM="$6"
DATABASE_PATH="$8"
...
while true; do
	...
	process_gtdbtk_output() {
		...
		if [ -f "$tsv_path" ]; then
			...
			awk 'BEGIN { print "Stamm,Count" } NR>1 { print $2 }' FS='\t' OFS=',' "$tsv_path" | sort | uniq -c | awk '{ print $2","$1 }' >>"$results_csv_path"
	...
	while IFS=, read -r STAMM COUNT; do
	...
	Rscript "$R_SCRIPT_DIR/plot_script.R" "$CSV_PATH" &
